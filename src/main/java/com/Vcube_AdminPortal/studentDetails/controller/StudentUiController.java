...
//	}
	...
	@GetMapping("/students-projects")
	public String showStudentsProjects(Model model) {
		model.addAttribute("projects", new ProjectModel());
	...
	public String getStudentDetails(@RequestParam String batchNo, @RequestParam String rollNo, Model model) {
		...
		try {
			...
			CompletableFuture<StudentModel> studentInfoFuture = CompletableFuture.supplyAsync(() -> studentDetails.getStudentsDetailsByBatchNoAndRollNo(batchNo, rollNo));
			CompletableFuture<AttendanceModel> attendanceInfoFuture = CompletableFuture.supplyAsync(() -> atda.getStudentAllAttendanceMonthsWithStatusByBatchRollNo(batchNo, rollNo));
			CompletableFuture<CaseStudyModel> caseStudiesInfoFuture = CompletableFuture.supplyAsync(() -> caseStudy.getStudentAllCaseStudiessWithStatusByBatchRollNo(batchNo, rollNo));
			CompletableFuture<ExamModel> examsInfoFuture = CompletableFuture.supplyAsync(() -> exam.getStudentAllExamsWithStatusByBatchRollNo(batchNo, rollNo));
			CompletableFuture<GUTExamModel> gutExamsInfoFuture = CompletableFuture.supplyAsync(() -> gutExam.getStudentAllGUTExamsWithStatusByBatchRollNo(batchNo, rollNo));
			CompletableFuture<MockModel> mocksInfoFuture = CompletableFuture.supplyAsync(() -> mock.getStudentAllMocksWithStatusByBatchRollNo(batchNo, rollNo));
			CompletableFuture<MockExamModel> mockExamsInfoFuture = CompletableFuture.supplyAsync(() -> mockExam.getStudentAllMockExamsWithStatusByBatchRollNo(batchNo, rollNo));
			CompletableFuture<ProjectModel> projectsInfoFuture = CompletableFuture.supplyAsync(() -> project.getAllProjectsDetailsWithStatusByBatchAndRolNo(batchNo, rollNo));

			CompletableFuture.allOf(studentInfoFuture, attendanceInfoFuture, caseStudiesInfoFuture, examsInfoFuture, gutExamsInfoFuture, mocksInfoFuture, mockExamsInfoFuture, projectsInfoFuture).join();

			StudentModel studentInfo = studentInfoFuture.join();
			AttendanceModel attendanceInfo = attendanceInfoFuture.join();
			CaseStudyModel caseStudiesInfo = caseStudiesInfoFuture.join();
			ExamModel examsInfo = examsInfoFuture.join();
			GUTExamModel gutExamsInfo = gutExamsInfoFuture.join();
			MockModel mocksInfo = mocksInfoFuture.join();
			MockExamModel mockExamsInfo = mockExamsInfoFuture.join();
			ProjectModel projectsInfo = projectsInfoFuture.join();
...
//			}
	...
	public String loadStudentsProjects(@RequestParam("batchNo") String batchNo, Model model) {
		List<ProjectModel> studentProjects = project.getStudentsProjectDetailsByBatchNo(batchNo);
		model.addAttribute("batchNo", batchNo);
		model.addAttribute("batches", getAvailableBatches());
		model.addAttribute("studentsProjects", studentProjects);
		return "students-projects";
	}
	...
	@ResponseBody
	public List<StudentModel> apiSearchStudents(@RequestParam(name = "query", required = false) String query,
			@RequestParam(name = "batch", required = false) String batch) {
		return studentDetails.searchStudents(query, batch);
	}

	@GetMapping("/api/batches")
	@ResponseBody
	public List<String> apiGetBatches(@RequestParam(name = "status", defaultValue = "active") String status) {
		if ("inactive".equalsIgnoreCase(status) || "in-active".equalsIgnoreCase(status)) {
			return BatchsListApp.getInactiveBatchesList();
		} else if ("all".equalsIgnoreCase(status)) {
			return BatchsListApp.getAllBatchesListRaw();
		} else {
			return BatchsListApp.getActiveBatchesList();
		}
	}

...
public class AttendanceController {
	...
	private Map<String,String> getStudentsRollNosWithNames(String batch) {
		return StudentsRollNosApp.getStudentsRollNosWithNameByBatchNo(batch);
	}
	...
	@ResponseBody
	public Map<String,String> getRollNosByBatch(@RequestParam String batchNo) {
		return getStudentsRollNosWithNames(batchNo);
	}
	...
	public String listStudents(@RequestParam(name = "batch", required = false) String batch,
		...
		if (selectedBatch != null && !selectedBatch.isBlank()) {
			...
			if (rollInput != null && !rollInput.isBlank()) {
	...
	public String submitAttendance(@RequestParam String rollNoSet, @RequestParam String batch,
		...
		try {
			AttendanceModel updateAttendance = atda.setAttendanceByRollNoSet(rollNoSet, batch, date);
...
//			atda.setEmailsForDailyAttendance(updateAttendance.getEmailsList(), batch);
	...
	public String submitAttendanceFromList(
		...
		if (students != null) {
			...
			atda.setEmailsForDailyAttendance(emails, batch);
		}

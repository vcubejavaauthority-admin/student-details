...
public class MockExamApp {
	...
	public String setMockExamByRollNoSet(MockExamModel atd,String batch) {
		...
		try {
			...
			String sheetName = "Mock-Exams"; // change if needed
	...
	public List<MockExamModel> getStudentsMockExamDetailsByBatchNo(String batch) {
		List<MockExamModel> mocks = new ArrayList<>();

		try {
			Sheets sheetsService = GoogleSheetsUtil.getSheetsService();
			String spreadsheetId =  BatchsListApp.getSheetIdbyBatchNo(batch);
			String sheetName = "Mock-Exams";
			...
			int totalMockExams = rows.get(0).size()-2;
			...
			for (List<Object> row : rows) {
				...
				stm.setName(stName);
				// Mock counts
				int studentTotalMockExams = 0;
				int studentTotalMockExamsMarks=0;
				...
				for (int c = 2; c < row.size(); c++) {
					...
					try {
						studentTotalMockExamsMarks += Double.parseDouble(val);
					} catch (NumberFormatException ex) {
						studentTotalMockExamsMarks += 0;
					}
					studentTotalMockExams++;
				}

				MockExamModel mock = new MockExamModel();
				mock.setBatch(batch);
				mock.setStudent(stm);
				mock.setStudentTotalMockExams(studentTotalMockExams);
				mock.setStudentTotalMockExamsMarks(studentTotalMockExamsMarks);
				if(studentTotalMockExamsMarks>0) {
					double percent = (studentTotalMockExamsMarks * 100.0) / (totalMockExams*40);
			...
			for (MockExamModel m : mocks) {
				m.setTotalMockExams(totalMockExams);
				m.setTotalMockExamsMarks(totalMockExams*40);
			}
	...
	public  MockExamModel getStudentAllMockExamsWithStatusByBatchRollNo(String batch, String rollNo) {
		MockExamModel exam = new MockExamModel();
...
//				System.out.println(dateHeader + " " + status);
			...
			exam.setDates(dates);
			exam.setMarks(marks);
			exam.setStudentTotalMockExams(studentTotalExams);
			exam.setStudentTotalMockExamsMarks(studentTotalExamsMarks);
			exam.setTotalMockExams(totalExams);
			exam.setTotalMockExamsMarks(totalExams * 40);

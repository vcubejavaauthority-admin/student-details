...
//				System.out.println(dateHeader + " " + status);
	...
	public List<StudentModel> getStudentsExamListByBatchAndDate(String batch, String uDate) {
		...
		try {
			...
			String examSheetName = "Exam";
			DateTimeFormatter inFmt = DateTimeFormatter.ofPattern("yyyy-MM-dd");
			DateTimeFormatter outFmt = DateTimeFormatter.ofPattern("dd-MM-yyyy");
			String dateStr = LocalDate.parse(uDate, inFmt).format(outFmt);
			System.out.println("DEBUG getStudentsExamListByBatchAndDate: batch=" + batch + ", dateStr=" + dateStr);

			String headerRange = examSheetName + "!A1:ZZ1";
			ValueRange headerResponse = sheetsService.spreadsheets().values().get(spreadsheetId, headerRange).execute();
			List<List<Object>> headerValues = headerResponse.getValues();
			...
			System.out.println("DEBUG getStudentsExamListByBatchAndDate: dateColIndex=" + dateColIndex);

			if (dateColIndex != -1) {
				int maxCols = dateColIndex + 10;
				String examRange = examSheetName + "!A2:" + indexToColumn(maxCols) + "1000";
				ValueRange examResponse = sheetsService.spreadsheets().values().get(spreadsheetId, examRange).execute();
				List<List<Object>> examRows = examResponse.getValues();
				if (examRows != null) {
					Map<String, String> examMarksMap = new HashMap<>();
					for (List<Object> row : examRows) {
						if (row == null || row.isEmpty()) {
							continue;
						}
						...
						if (!roll.isEmpty()) {
							String mark = row.size() > dateColIndex && row.get(dateColIndex) != null
									...
									: "";
							examMarksMap.put(roll.toLowerCase(), mark);
						}
					}

					for (StudentModel st : students) {
						String mark = examMarksMap.get(st.getRollNo().toLowerCase());
						if (mark != null) {
							st.setMark(mark);
						}
					}
	...
	public String saveExamMarks(String batch, String uDate, List<String> rollNos, List<String> marks) {
		DateTimeFormatter inFmt = DateTimeFormatter.ofPattern("yyyy-MM-dd");
		DateTimeFormatter outFmt = DateTimeFormatter.ofPattern("dd-MM-yyyy");
		String dateStr = LocalDate.parse(uDate, inFmt).format(outFmt);
		System.out.println("DEBUG saveExamMarks: batch=" + batch + ", dateStr=" + dateStr);
		System.out.println("DEBUG saveExamMarks: rollNos=" + rollNos);
		System.out.println("DEBUG saveExamMarks: marks=" + marks);
		...
		try {
			...
			System.out.println("DEBUG saveExamMarks: dateColIndex=" + dateColIndex);

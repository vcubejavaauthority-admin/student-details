...
//				stm.setOnline(onlineEmails.contains(stEmail));
	...
	public List<StudentModel> getStudentsFromStudentInfoByBatch(String batch) {
		List<StudentModel> students = new ArrayList<>();
		if (batch == null || batch.isBlank()) {
			return students;
		}
		...
		try {
			...
			for (List<Object> row : rows) {
				...
				String stRollNo = row.size() > 0 ? row.get(0).toString().trim() : "";
				String stName = row.size() > 1 ? row.get(1).toString().trim() : "";
				String stPhotoURL = row.size() > 2 ? row.get(2).toString().trim() : "";
				String stMobile = row.size() > 3 ? row.get(3).toString().trim() : "";
				String stEmail = row.size() > 4 ? row.get(4).toString().trim() : "";
				String stStatus = row.size() > 5 ? row.get(5).toString().trim() : "";
				...
				StudentModel stm = new StudentModel();
				stm.setBatchNo(batch);
				stm.setRollNo(stRollNo);
				stm.setName(stName);
				stm.setPhotoURL(stPhotoURL);
				...
				stm.setStatus(stStatus);

				students.add(stm);
			}
		} catch (Exception e) {
			System.err.println("Error reading Student-Info for batch " + batch + ": " + e.getMessage());
		}

		return students;
	}

	public List<StudentModel> searchStudents(String query, String selectedBatch) {
		List<StudentModel> allStudents = new ArrayList<>();
		
		if (selectedBatch != null && !selectedBatch.isBlank()) {
			allStudents = getStudentsFromStudentInfoByBatch(selectedBatch);
		} else {
			List<String> batches = BatchsListApp.getAllBatchesList();
			if (batches != null && !batches.isEmpty()) {
				List<CompletableFuture<List<StudentModel>>> futures = batches.stream()
						.map(batch -> CompletableFuture.supplyAsync(() -> getStudentsFromStudentInfoByBatch(batch)))
						.toList();

				CompletableFuture.allOf(futures.toArray(new CompletableFuture[0])).join();

				for (CompletableFuture<List<StudentModel>> future : futures) {
					try {
						allStudents.addAll(future.get());
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
		...
		return allStudents.stream()
				.filter(s -> {
					String status = (s.getStatus() != null) ? s.getStatus().trim().toLowerCase() : "";
					return status.equals("active") || status.equals("online") || status.equals("placed") 
							|| status.equals("drop") || status.equals("add-on");
				})
				.filter(s -> {
					if (cleanQuery.isEmpty()) {
						return true;
					}
					boolean nameMatches = s.getName() != null && s.getName().toLowerCase().contains(cleanQuery);
					boolean emailMatches = s.getEmail() != null && s.getEmail().toLowerCase().contains(cleanQuery);
					boolean mobileMatches = s.getMobile() != null && s.getMobile() != 0L && String.valueOf(s.getMobile()).contains(cleanQuery);
					return nameMatches || emailMatches || mobileMatches;
				})
				.toList();
	}

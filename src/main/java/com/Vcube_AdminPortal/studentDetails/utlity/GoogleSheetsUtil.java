...
public class GoogleSheetsUtil {
    ...
    /**
     * Builds and returns an authorized Sheets client service using a service account.
     * Make sure the credentials file exists at:
     * src/main/resources/credentialKeys/vcube-admin-portal-credential.json
     * and that the sheet is shared with the service-account email.
     */
    ...
    /**
     * Resolves the HTML5 date input (yyyy-MM-dd) to Google Sheets expected format (dd-MM-yyyy).
     * If date is empty or invalid, defaults to today's date.
     */
    ...
    public static int findOrInsertDateColumn(Sheets sheetsService, String spreadsheetId, String sheetName, String todayStr, int dateStartIndex) throws IOException {
        String headerRange = sheetName + "!A1:ZZ1";
        com.google.api.services.sheets.v4.model.ValueRange headerResponse = sheetsService.spreadsheets().values().get(spreadsheetId, headerRange).execute();
        ...
        // Shift columns using Google Sheets API (always insert if not appending at the very wide end)
        com.google.api.services.sheets.v4.model.Spreadsheet ss = sheetsService.spreadsheets().get(spreadsheetId).execute();
        ...
        com.google.api.services.sheets.v4.model.InsertDimensionRequest insertCol = new com.google.api.services.sheets.v4.model.InsertDimensionRequest()
                .setRange(new com.google.api.services.sheets.v4.model.DimensionRange()
                        .setSheetId(sheetId).setDimension("COLUMNS").setStartIndex(insertIndex).setEndIndex(insertIndex + 1));
        
        if (insertIndex > 0) {
            insertCol.setInheritFromBefore(true);
        }
        
        com.google.api.services.sheets.v4.model.Request req = new com.google.api.services.sheets.v4.model.Request().setInsertDimension(insertCol);
        com.google.api.services.sheets.v4.model.BatchUpdateSpreadsheetRequest batchReq = new com.google.api.services.sheets.v4.model.BatchUpdateSpreadsheetRequest().setRequests(Collections.singletonList(req));
        ...
        sheetsService.spreadsheets().values().update(spreadsheetId, dateCellRange, dateBody)
